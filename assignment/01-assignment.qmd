...
<!-- # post-F25, I updated the exercises to have tryi-it callout boxes as in Assignment 2 -->
...
If you have not yet done so, you'll need to install both `R` and RStudio. See the [Installing page of our course resources](/resource/install.html) for instructions.


## If you totally forgot how to use `R`

If you need a refresher on the basics of `R`, work through the [Using R](/resource/usingR.qmd) resource before starting this lab.
...
For more on using R Markdown, see the [R Markdown page of our course resources](/resource/rmarkdown.html). You can also learn about the general Markdown language [on our Markdown page of our course resources](/resource/markdown.html)
...
10. Repeat exercise 9, but this time use `sapply`.

11. Repeat exercise 8, but this time use `map_dbl`.

12. Create a data.frame holding $n$ and $s_n$ (from either 9, 10, or 11). Plot $s_n$ versus $n$. Use points defined by $n=1,\dots,25$.

13. Confirm that the equivalent formula for this sum is $S_n= n(n+1)(2n+1)/6$. To do so, compare whether `compute_s_n` gives the same result for the vector `s_n` as the equivalent formula for the sum. 
