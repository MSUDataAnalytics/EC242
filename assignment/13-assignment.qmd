...
- [<i class="fas fa-file-csv"></i> `booking.csv`](/data/Lab_booking.csv) - Contains the corporation name, the room type, and the dates someone from the corporation stayed at the hotel. It was pulled by an intern who doesn't understand date-time stamps. 

- [<i class="fas fa-file-csv"></i> `roomrates.csv`](/data/Lab_roomrates.csv) - Contains the price of each room on each day. The Lab 11 version of this data is from our German affiliate, so pay attention to the date format, and be careful -- it's very wide, so using `head` will throw a lot of data at you.

- [<i class="fas fa-file-csv"></i> `parking.csv`](/data/Lab_parking.csv) - Contains the corporations who negotiated free parking for employees. It has been updated.
