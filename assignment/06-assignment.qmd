---
title: "6: Statistical Models and Uncertainty"
...
One note on the story below, which is adapted from Rafael Irizarry's *Introduction to Data Science*. FiveThirtyEight is the hero of it, and at the time that is genuinely how it looked. FiveThirtyEight is also gone --- ABC shut it down in March 2025 and pointed the domain at abcnews.com. Nate Silver had left two years before that and now runs the same model out of a Substack. The model outlived the newsroom. Being right is apparently not the same as being kept around.
...
8. Report your estimate of $b_2 - b_1$ in percentage points and say whether zero falls inside your 95% interval. Then go further. Compute each pollster's average spread on its own and compare both to the actual election night result, a 2.1% Clinton win. Which of the two was closer? Now average the two pollster averages together and compare *that* to 2.1%. Your boss can afford a data subscription to exactly one of these two shops. In two or three sentences, tell her which one to buy --- or tell her why that is the wrong question.
