---
title: "11: Nonlinear/Nonparametric Models"
...
<!-- NOTE: data/bank23.csv's column named `default` is NOT default-on-loan. It is byte-identical to bank.csv's `y` (term deposit subscribed): 521 yes / 4000 no, 0 mismatches across all 4,521 rows. The real default column (76 yes) was dropped when this file was cut. The backstory below is written around that. -->
...
You work for a bank. Specifically, you work in the corner of the bank that calls people during dinner to sell them a term deposit. Somebody has to.

They have handed you the records of 4,521 of these phone calls. The column recording whether the person on the other end eventually said yes is called `default`. Yes, `default`. No, it has nothing to do with defaults on loans, and no, nobody here can tell you why. Welcome to working with data somebody else collected and named. Your first act should be to rename it to something honest.

About one call in nine ends in a sale. Your manager would like to stop making the other eight, and she has heard that you know k-nearest neighbors. So `default` -- whatever it should have been called -- is your *target* variable.
...
2. kNN needs to make a numeric prediction. Since `default` (the sale indicator, remember) is not numeric, make a new column for it that is numeric. Of course, you'll need to encode the numbers in a meaningful way (`as.numeric('no')` will do you no good).
