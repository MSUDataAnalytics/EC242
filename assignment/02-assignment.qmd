...
For Exercise 3, use this [data on MI county-level income, age, population, and commute time](data/milong.csv), which can be installed using the following line of code to directly create the object in your code chunk:
...
Then, categorize each county into "small" and "large" based on population. Do the same for age ("young" and "old"). A logical cut-point would be the median or mean.

Then, plot the relationship between *two of the variables of your choice* using the geometry of your choice. Use an aesthetic mapping on the young/old **or** small/large category to illustrate how the relationship may differ. If you're aesthetic mapping young/old to an aesthetic, do not also use it on the X or Y axis (that would be redundant).
