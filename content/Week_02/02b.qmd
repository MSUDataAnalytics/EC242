...
Your job is to rebuild the heatmap above for the modern era. About fifteen
minutes. Steal the colour palette from the figure above --- it's in the source of
this page, and stealing a palette that works is a perfectly respectable thing to do.

1. Plot `date` on x, `state` on y, and map `new_cases` to the `fill` aesthetic
   with `geom_tile()`.
...
Then answer two questions, because a plot that doesn't answer anything is
just decoration:

- **Which three states would you point at?** Name them and say what's different
  about each one --- a single sharp burst is a different story from a long smoulder.
