...
There are other lots of neat tricks for axis labels on the tidyverse [scales vignette](https://tidyverse.org/blog/2022/04/scales-1-2-0/)

## Axis tick label orientation

While the `label_comma` kindly leaves room for us on the x-axis breaks (by eliminating some of the tick labels), we can also alter the orientation of the labels. This occurs in the `theme()` call since it adjusts the layout or styling of the plot. Here, I've put in a bunch of breaks at 1 million to 40 million, and then asked `ggplot` to rotate and adjust the x-axis tick labels. `hjust` and `vjust` move the orientation of the output. `angle` changes the orientation.
...
You might find these code bits to be useful for this (hint: use them with `case_when`):
