...
We can use the `labels = label_number(scale_cut = cut_short_scale())` argument to shorten the display and use the common suffix.
...
There is also a `cut_si()` function that takes as an input a scientific unit (like "g" for grams). It'll use the full scientific notation for larger quantites, so `cut_si("g")` would have "g", then "kg", then "Mg" where needed on the axis.
