...
library(forcats)    # Operations on factors (reordering, etc.)
```
...
The foundation of linear regression is correlation. We can visualize the correlations between pairs of variables with the `ggpairs()` function in the **GGally** package. For instance, how correlated are high and low temperatures, humidity, wind speed, and the chance of precipitation? We first make a smaller dataset with just those columns, and then we feed that dataset into `ggpairs()` to see all the correlation information:
...
Here's some code to load data from the Residential Energy Consumption Survey (RECS), a [periodic survey of household energy consumption done by the Energy Information Administration.](https://www.eia.gov/consumption/residential/data/2009/). Use the code here to load the data. I've selected a few variables and renamed them to something intuitive. Each observation is a household surveyed by the EIA. The variable named `EnergyUsed` is the total energy consumed by that household, in thousands of BTUs.

Most of the variables are self-explanatory except the following:

- `AgeOfHome` is on a 1-8 scale where 1 is the oldest (built before 1950) and 8 is the newest (built 2005-2009). This is "discretized" age.

- `HeaterAge` is the age of the home's main heating equipment, and EIA's codes are **not** in numeric order: 1 = less than 2 years old, 2 = 2-4 years, 3 = 5-9 years, 41 = 10-14 years, 42 = 15-19 years, and 5 = 20 years or older. Recode it before treating it as a number, or the 41s and 42s will wreck your correlation.

- `TVSize` is on a scale of 1 (smallest) to 3 (largest)

- `TempHome` tells us the home's winter thermostat setting when someone is home (in degrees Fahrenheit)

- `TreeShade` and `HasAC` are 0/1: 1 means the home is shaded by trees / has air conditioning


One warning about the codes: `CarsGarage` and `TVSize` also use `-2` for "not applicable" (no attached garage, no TV), and the filter below only drops the `-2`s in `HeaterAge` and `TempHome`. Almost 60% of households have `CarsGarage == -2`, so if you correlate that variable as-is you are mostly measuring "has a garage at all."
...
Instead of using a heatmap, we can also use points, which encode the correlation information both as color *and* as size. To do that, we switch `geom_tile()` to `geom_point()`, switch the `fill` aesthetic to `color` (and `scale_fill_gradient2()` to `scale_color_gradient2()`), and set the `size = abs(cor)` mapping (note the use of the absolute value function):
...
First, let's filter the Atlanta weather data to only look at the summer. We also add a column to scale up the humidity value—right now it's on a scale of 0-1 (for percentages), but when interpreting regression we talk about increases in whole units, so we'd talk about moving from 0% humidity to 100% humidity, which isn't helpful, so instead we multiply everything by 100 so we can talk about moving from 50% humidity to 51% humidity. We also scale up a couple other variables the same way, in case you want to put them in a model of your own.
...
$$HighTemp_i = \beta_0 + \beta_1 HumidityScaled_i + \varepsilon_i$$

Each of our `r NROW(weather_atl_summer)` observations is indexed by $i$. Since it will almost never be the case that $\beta_0 + \beta_1 HumidityScaled_i = HighTemp_i$, we include the error term $\varepsilon_i$ to account for things that aren't in our regression. 
...
$$E[HighTemp|HumidityScaled] = \beta_0 + \beta_1 HumidityScaled$$

Note that we do not have an error term here -- the $E[]$ doesn't have an error term in it, only the equation for an individual observation $i$ does.

To do this in R, we'll first use the formula from [Week 07a](/content/Week_07/07a.html#the-regression-line):
...
Note that the formulas from [Week 07a](/content/Week_07/07a.html#the-regression-line) have a direct translation into the code. Also note that we used `summarize` on the data so that we get only **one** row of data (no more `r NROW(weather_atl_summer)` rows, we've summarized them using `mean` and `sd`).
...
- `beta_0` (or $\beta_0$) is the intercept. By definition of an intercept, it shows that the average high temperature when there's **0% humidity** is 104°. There are no days with 0% humidity though, so we can ignore the intercept—it's really just here so that we can draw the line.

- `beta_1` (or $\beta_1$) is the coefficient for `HumidityScaled`. It is the slope of the regression line, and thus the change in E[HighTemp] per 1-unit increase in $HumidityScaled$. It shows that a one percentage point increase in humidity is **associated with** a 0.241° decrease in the high temperature, on average.
...
Visualizing this model is simple, since there are only two variables. We visualized the regression line in the Galton Heights data during [Week 07a](/content/Week_07/07a.html#the-regression-line), so let's just use that code.
...
Every point along the regression line is the $E[HighTemp|HumidityScaled]$. Put in whatever value of humidity you want, and the line tells you what the expectation of the high temperature will be.
...
We'll have you share your work and interpret the results when done -- use our {{< var url.zoom >}} room.
