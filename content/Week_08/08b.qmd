...
First, note that `am` is already a dummy variable, since it uses the values `0` and `1` to represent automatic and manual transmissions. Often, a variable like `am` would store the character values `auto` and `man` and we would either have to convert these to `0` and `1`, or, as we will see later, `R` will take care of creating dummy variables for us. In the plot above, we use `as.logical`, which takes 0/1 and makes the `FALSE/TRUE`, which are treated as factor variables for plotting.


### Its just a dummy....
We've noticed that most of the blue dots (with manual transmission) have higher `mpg`. We have a dummy in our data. Taking a step back from using `hp` to explain variation in `mpg`, let's look at how much `am` does on it's own:
...
We've established what the dummy variable does on it's own. Now we'll use it in a multivariate regression. To fit a more complex model using the dummy and `hp`, we do so like any other multiple regression model we have seen before.
