...
### Some new packages
We'll be using `tmaptools` geocoder today, so you'll need to install that package. And `tigris` if you didn't already.
...
What if we wanted to find the closest gas plant to each city? That is akin to looking at each row, and finding the column that is the smallest, right? We will use `apply`, and we will note that the order of the columns is the same as the order in `MI.gasplants.small`, so we can use `MI.gasplants.small$name` to tell us the name of the closest gas plant. We will `apply` over each row (`MAR=1`) and use the `which.min` function, which returns the *index* number of the minimum column.
...
### st_nearest_feature
As is common in R, there is a function that will get the closest points between to spatial objects. `st_nearest_feature` takes two geometries, and returns  the neaarest point in `y` for every point in `x`, which is what we did with the MI gas plants.
...
To do so, we use `st_buffer()` along with a projection that specifies distance in some linear measure (rather than degrees lat or lon. This is because a degree of longitude *changes* when we get closer to the poles. Let's find all counties in Michigan that are within 50 km of any Great Lake. To do so, we first buffer our Great Lakes by 50km. The `st_crs()` of the `GL` object is already stated in meters, so we can buffer by 50,000:

```{r}
st_crs(GL) # "LENGTHUNIT" is metre
...
What happened? For each Great Lake within 50km, a county in the merged data got one row, so many counties got two (or more) rows:
