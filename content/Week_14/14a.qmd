...
library(here)
library(knitr)
include_graphics(here('images','classification.png'))
```
...
OK, that's 10 "No" in a row. Is it all "No"?

```{r}
table(default_tst_pred)
```
...
In the table above, the **False Positives** are the observations for which we *predicted* "Yes" but were actually observed to be "No". There are `r tst_tab['Yes','No']` False Positives.

::: {.callout-note}
# Question

How many False Negatives above?
...
The `confusionMatrix()` function from the `caret` package can be used to obtain a wealth of additional information, which we see output below for the test data. Note that we specify which category is considered "positive." Here, "Yes" and "No" for default are pretty obvious, but if your binary target is not so clear, it's helpful to fix which outcome is "positive".
...
Here, $I$ is an indicator function, so we are essentially calculating the proportion of predicted classes that do **not** match the true class.
...
Like regression, we often split the data and then consider Train (Classification) Error and Test (Classification) Error. The test error is our measure of how well a classifier will work on unseen future data.
...
We can go back to the test confusion matrix from before (called `tst_tab`) and hand-calculate the error rate
```{r}
...
Sometimes guarding against making certain errors, FP or FN, is more important than simply finding the best accuracy. Thus, sometimes we will consider **sensitivity** and **specificity**.
...
Here, we see an extremely low prevalence, which suggests an even simpler classifier than our current one based on `balance`.
...
We also re-split the data. Note the seed is different from the one we used above, so this is a *different* split -- run this chunk even if you still have the old `default_trn` and `default_tst` sitting around.
...
Everything seems to be working. Let's plot the results.
...
Above, $\sigma$ is a function, the **sigmoid** function. It is shorthand notation which appears often in deep learning literature. It takes any real input, and outputs a number *between* 0 and 1. How useful! (This is actually a particular sigmoid function called the logistic function, but since it is by far the most popular sigmoid function, often sigmoid function is used to refer to the logistic function.)
...
We start with a single predictor example, again using `balance` as our single predictor. Note that `default_trn` has a factor variable for `default` (No/Yes). `R` represents factor variables as numbers (here, 1 and 2), but it is easy to lose track of which number is which string. By default `R` sorts the labels and assigns 1 to the first alphabetically, 2 to the second, etc., which is why `No` is 1 and `Yes` is 2 -- but re-make the factor with different labels, or with an explicit `levels =` argument, and that mapping can switch.

It's best to be explicit. Let's set the `default` variable to be `Yes = 1` and `No = 0`. Then it's a true binary, and `mean(default_trn$default)` gives us the average default rate (in the training set). We *should* do this before we split the sample, but we can do it now to both samples as long as we do the exact same thing to each.

```{r, collapse = F}
default_trn = default_trn %>%
  dplyr::mutate(default = case_when(default=='No' ~ as.integer(0),
                                    default=='Yes'~ as.integer(1)))

default_tst = default_tst %>%
  dplyr::mutate(default = case_when(default=='No' ~ as.integer(0),
                                    default=='Yes'~ as.integer(1)))

```
Now we have no ambiguity about what we are predicting.
...
we would have to run those link values through the sigmoid function ourselves. We could do that, or we could just use `type = "response"`
...
```{r, message = FALSE, warning = FALSE, collapse = F}
...
get_logistic_error(model_glm, data = default_trn,
                   res = "default", pos = 1, neg = 0, cut = 0.5)
```
...
This is equivalent to points that satisfy
...
model_3 = glm(default ~ . ^ 2,
              data = default_trn, family = "binomial")
model_4 = glm(default ~ . ^ 3 + poly(balance, 3),
              data = default_trn, family = "binomial")
```
...
Here we see the misclassification error rates for each model. The train (weakly) decreases, and the test decreases, until it starts to increase. Everything we learned about the bias-variance tradeoff for regression also applies here.
...
We write a function which allows us to make predictions based on different probability cutoffs.
...
We see that sensitivity decreases as the cutoff is increased. Conversely, specificity increases as the cutoff increases. This is useful if we are more interested in a particular error, instead of giving them equal weight.
...
## Multinomial Logistic Regression (BONUS)
...
Notice we are only given coefficients for two of the three classes, much like only needing coefficients for one class in logistic regression.
...
Interestingly, you've just fit a neural network, and you didn't even know it! (Hence the `nnet` package.)
