...
We can go back to the `tst_con_mat` table (called `tst_tab`) before and hand-calculate the error rate
```{r}
...
Above, $\sigma$ is a function, the **sigmoid** function. It is shorthand notation which appears often in deep learning literature. It takes any real input, and outputs a number *between* 0 and 1. How useful! (This is actualy a particular sigmoid function called the logistic function, but since it is by far the most popular sigmoid function, often sigmoid function is used to refer to the logistic function)
