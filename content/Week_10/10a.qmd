...
Your lab this week uses the Ames housing data, so grab it now. The lecture itself runs on simulated data plus whatever ships with the `dslabs` and `HistData` packages, so there's nothing else to download.
...
and so on. How you do so is up to you -- you could do a list of formulas constructed with `paste`, then use `lapply` on that list. Or you could wisely use the `~ .` part of the formula and change the columns in the data you pass to `lm`. Here are a few tips pertaining to some of the ways of doing this.
...
The first column denotes group. We created `r prettyNum(g, big.mark=",", scientific=FALSE)` groups and for each one we generated a pair of independent vectors, $X$ and $Y$, with `r N` observations each, stored in the second and third columns. Because we constructed the simulation, we know that $X$ and $Y$ are not correlated.
...
It's just a mathematical fact that if we observe `r prettyNum(g, big.mark=",", scientific=FALSE)` random correlations that are expected to be 0, but have a standard error of `r sd(res$r)`, the largest one will be enormous.
...
The chi-squared test compares two groups with binary outcomes (like "admit" and "nonadmit"). The null hypothesis is that the groups are not differently distributed between the outcomes. Here's what the data looks like going in -- it's in counts of "admitted" and "not admitted".
...
Four out of the six majors favor women. The one big gap is major A, where women are admitted 20 points *more* often than men -- which runs backwards from the headline. Every other difference is much smaller than the 14.2 difference that we see when examining the totals.
...
If we average the difference by major, we find that the percent admitted is actually 3.5 points higher for women.
...
The case we have just covered is an example of Simpson's paradox. It is called a paradox because we see the sign of the correlation flip when comparing the entire population and specific strata. As an illustrative example, suppose you have three random variables $X$, $Y$, and $Z$ and that we observe realizations of these. Here is a plot of simulated observations for $X$ and $Y$ along with the sample correlation:
...
Next time, we'll formally cover defining our loss function (root mean squared error) and using it to tune a model. But for now, let's compare the two models above -- the "parsimonious" one where we let $Z$ be an intercept shift, and the more complex "flexible" model where we interact $X$ with $Z$ (treating $Z$ as a factor). 

First, we'll re-estimate the models and get the residuals. We could use the difference between observed $y$ and the predicted $y$, or just use `resid(...)` to get the same thing:

```{r}
parsimonius = lm(y ~ x + as.factor(z), data = dat)
pars.resid = dat$y -  predict(parsimonius)
pars.resid = resid(parsimonius)  # same thing

flexible = lm(y ~ x*as.factor(z), data = dat)
flex.resid = resid(flexible)

```

By construction, they will have mean = $0$. Why?
...
Usually we'd subtract off the mean of the numbers to get to the dispersion of the errors around the mean. It's already 0, so we skip that part.

Now, we square them, sum them up, and then divide by the number of observations to get a **mean squared error**

```{r, echo=T}
pars.RSS = sum(pars.resid^2)
print(pars.RSS)
pars.RSS/length(pars.resid)

flex.RSS = sum(flex.resid^2)
print(flex.RSS)
flex.RSS/length(flex.resid)
```


And for root mean squared error, we finally take the square root. This looks an awful lot like the standard deviation of the residual, right? It just about is (aside from the sample $\frac{1}{N-1}$ difference).

```{r, echo=T}
sqrt(sum(pars.resid^2)/length(pars.resid))
sqrt(sum(flex.resid^2)/length(flex.resid))
```

Which is smaller?

Previously, we used `anova()` to test to see which nested model is better. But **better** is defined in a specific way: are the additional interactions doing anything in a statistical sense. Formally, `anova()` performs an F-test to statistically test whether or not the interactions are jointly zero. We can use it here.

```{r, echo=T}
anova(parsimonius, flexible)
```

We **fail to reject the null that the interactions are jointly equal to zero**.

### But....

What if we were only interested in the best prediction, not necessarily the best *model*? Then "**better**" is defined as "lower RMSE" and the more flexible model is "better".

Starting next time, we'll look at what we can learn when we aim only for a predictive model rather than a theoretically valid, explanatory one.
