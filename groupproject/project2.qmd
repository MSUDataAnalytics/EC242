...
- You *show* your code in 1, but suppress your code using `echo=F` in 2-5.

- You will need to use `lapply` or a loop, and not the built-in cross-validation, for finding the optimal test RMSE. You can use `glmnet`, for instance, to estimate a LASSO model, but you have to use `lapply` to vary the value of the tuning parameter `lambda` rather than giving it a range of values of `lambda`.
...
- [You are required to turn in both your final, rendered project with no code showing AND your .RMD file used to render the project. All code generated with generative AI (e.g. ChatGPT) must be labeled as such as required in the syllabus.]{.text-red}. I have updated this in the assignment below.
...
You will turn in both your final rendered project **and** your .RMD file.
