...
4. **Evaluate the Model** (8 points): Take your model to the evaluation sample (see below), calculate the RMSE for your optimal (lowest *test* RMSE) model on this sample, and report it (2 points). Then, see if your priors were correct by changing the variable you hypothesized to be related, and see if your predictive model changes it's prediction accordingly. For instance, I think having air conditioning `AIRCOND` will increase the energy consumed. To see if this holds in my model, I can increase `AIRCOND` from 0 to 1 (since it is a categorical variable; use a percent or a fixed amount for any continuous variable) across all the data, and compare the new prediction to the old. Do this for each of the three prior predictions you stated in (2). (6 points) 
