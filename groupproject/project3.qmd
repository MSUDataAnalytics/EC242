...
The goal of this project is to predict household energy consumption using the Department of Energy's *Residential Energy Consuption Survey* for 2020 using one of the predictive models you have learned: LASSO, elastic net, ridge, KNN, or trees (but not OLS). The RECS is a sample of around 18,000 households that asks myriad questions about housing characteristics (age of house, size of house), appliances (heat pump, gas furnace, EV), and total energy consumption (measured in BTU's such that electricity, gas, propane, etc. are all included). This is *microdata*, so each household is an observation in the data.
...
This is a new-ish project, so there may be some unclear descriptions or details that are necessary. I'll add questions and answers here as they come in.
...
3. **Estimate a model** (20 points): Estimate a robust and detailed predictive model that includes the relationship from (2) and anything and everything else you think will provide predictive power -- the sky is the limit. Use one of the machine learning methods that we learned about (kNN, regression trees, LASSO/Ridge/Elastic Net, do **not** use regular OLS) and carefully tune your model using the train-test paradigm to arrive at your best prediction.
  ...
  - Most of methods we learned start out with a specification formula that is highly complex -- unlike when we added variables to change complexity, our machine learning methods take a very deatiled formula possibly with polynomial terms and interactions, and use the tuning parameters to change the complexity of the model. So *start with a very complex model* in your formula.

  - As you adjust your tuning parameters, you may find that a large part of the tuning parameter space (the values of the tuning parameters you're trying) result in a flat, high RMSE. As we discussed in class, there is no "right" range for the tuning parameters, so you may need to adjust the range you cover so that you focus on the RMSE-minimizing values. If you're in a tuning parameter range that results in very high RMSE, then your plot is going to "compress" the minimizing point down and you likely won't be able to even tell there's a minimum. Be smart and iterate the range for your tuning parameter estimates.

  In the write-up for this section (section 3), **include a table** showing the range of *complexity* that was tried (e.g. the *lambda* from a LASSO), the training RMSE, and the testing RMSE. The table will be long, but that's OK. If you have more than 100 different values, then shorten the range around the test-RMSE-minimizing point. Use `round()` to cut down on the number of digits showing, but make sure enough are showing to find the minimizing level of complexity. Then, **make a plot** showing the progression of both RMSE's as complexity increases. The plot should clearly show the nadir where the testing RMSE reaches it's minimum and that point should be labeled. Complexity should be increasing along the x-axis, even if the tuning parameter is decreasing as complexity increases. If you are using a method that has two complexity parameters, you'll need to include test and train RMSE's for every combination of parameter values. In that case, choose a coarse number of parameter values in the complexity parameters so that you are not searching over too many values. You can plot across one complexity parameter with the other complexity parameter held fixed. Your write-up should **not** include any code in this section, just the table and the plot. You can try using `knitr::kable()` to output a table - this function converts data.frame-like R objects into clean tables for RMarkdown. We haven't covered it in class, but it should be easy to figure out.

  Finally, in a 2-3 sentence discussion, note which model you have selected as your optimal model and why. 

4. **Evaluate the Model** (8 points): Take your model to the evaluation sample (see below), calculate the RMSE for your optimal (lowest *test* RMSE) model on this sample, and report it (2 points). Then, see if your priors were correct by changing the variables you hypothesized to be related, and see if your predictive model changes it's prediction accordingly. For instance, I think having air conditioning `AIRCOND` will increase the energy consumed. To see if this holds in my model, I can increase `AIRCOND` from 0 to 1 (since it is a categorical variable; use a percent or a fixed amount for any continuous variable) across all the data, predict the new consumption, and compare the new prediction to the original. Do this for each of the three prior predictions you stated and plotted in (2). (6 points) 
...
- Drop *anything* starting with the letter *Z* -- these are indicators that the variable following the *Z* was imputed (algorithmically guessed). You can explore the tidyverse selecter `starts_with()` to help with this.

- Drop *anything* starting with *NWEIGHT* as these are survey weights we don't have to worry about.

- Drop *anything* starting with *KWH* or *BTU* or *DOL* or *CUFEET* or *GALLON* as these are subcategories of energy consumption reported, so obviously they'd perfectly predict total consumption.

- Drop `TOTALBTUSPH`, `TOTALDOLSPH`, `TOTALBTUWTH`, `TOTALDOLWTH`,`TOTALBTUOTH`,`TOTALDOLOTH`, `TOTALDOL` and `DOEID` as these are subcategories of total energy usage. But make sure you keep your target variable, `TOTALBTU`!
...
- Ten of the surviving columns carry `NA`s: `MEDICALDEV` (210 rows) plus `EVCHRGHOME`, `EVCHRGWKS`, `EVCHRGBUS`, `EVCHRGMUNI`, `EVCHRGDLR`, `EVCHRGHWY`, `EVCHRGOTH`, `EVHOMEAMT` and `EVCHRGTYPE` -- 290 incomplete rows in all. `glmnet` refuses an `x` matrix containing them outright. `knnreg` is worse: it quietly drops the incomplete rows and hands you back a prediction vector shorter than the data you predicted on, so your RMSE comes out wrong with nothing more than a warning. Drop those ten *columns* -- not the rows -- so that the row counts below still work out. (If you would rather drop the rows with `na.omit()`, say so in your write-up and report your actual sample sizes, which will be smaller than the numbers below.)

- Use `set.seed(2422025)` and then immediately do the following:

  - Sample exactly 1,000 households to hold as the final evaluation data. This is neither test nor train, but a third holdout "evaluation" set with which we will use to evaluate the final model.
...
- Do your three priors and their description make sense?
...
- Does complexity *increase* along the x-axis?
...
[You are required to turn in both your final, rendered project as a PDF (RMarkdown rendered to PDF using LaTeX, as usual) with no code showing (except as stated in Section 1) AND your .RMD file used to render the project. All code generated with generative AI (e.g. ChatGPT) must be labeled as such as required in the syllabus.]{.text-red}.
