...
NYC keeps incredibly detailed data regarding animal sightings, including rats, and it makes this data publicly available through 311. (Note the source: NYC Open Data, not Kaggle. See Part 2 for why I care.) The city has since split its 311 archive in two: [2020 to present](https://data.cityofnewyork.us/Social-Services/311-Service-Requests-from-2020-to-Present/erm2-nwe9) and [2010 to 2019](https://data.cityofnewyork.us/Social-Services/311-Service-Requests-from-2010-to-2019/76ig-c548). The file below already stitches the two together, so you do not need either link for Part 1 --- but you will if you want to go back for more.
...
One thing worth knowing before you start. Rat sightings roughly doubled between 2013 and 2022. In April 2023 the city responded by appointing a Director of Rodent Mitigation --- the press immediately called her the rat czar --- and phasing in rules requiring commercial and residential waste to go out in lidded containers rather than bags on the curb. The data below ends on August 23, 2026, so it covers the whole arc: the rise, the intervention, and whatever has happened since. One trap to avoid: 2026 is only two-thirds of a year in this file, so a plot of sightings per year will show a cliff at the right edge that is mostly an artifact of when I pulled the data rather than a collapse in the rat population. Compare like with like --- restrict to comparable months across years, or annualize. Whether any of that shows up in 311 calls is a question you can actually answer, and "it didn't" is a perfectly good finding.
...
6. **Upload** the following outputs to D2L:
    ...
    - A PDF file of your memo with your graphics embedded in it.^[You can approach this in a couple different ways—you can write the memo and then include the full set of figures at the end, [similar to this blog post](https://rud.is/b/2017/09/18/mapping-fall-foliage-with-sf/), or you can write the memo in an incremental way, describing the analytical steps that each figure illuminates, ultimately arriving at a final figure, [like this blog post](https://github.com/rudeboybert/fivethirtyeight).] This means you'll need to do all your coding in an `R` Markdown file and embed your code in chunks. 
...
- Your code should **not** appear in your output - it should be only your plots and memo writing. 
  - To turn off code echoing, add `echo = FALSE` in each of your code chunk options (e.g. `{r setup, echo = FALSE}`), or set it globally in the first code chunk using `knitr::opts_chunk$set(echo = FALSE)` (you'll still have to add `echo=F` to that code chunk though).
...
- Twenty rows have no usable borough (18 marked "Unspecified", two blank), so I filter them out. The blanks become `NA` under the `na =` argument above, and `filter()` drops those too.
...
Start this part early. We cover data hunting in Week 5, and the bulk of it lands Thursday, October 1 --- days before this memo is due. Do not wait on lecture to start looking.

For the second part of the project, your task is simple. Your group must identify three different data sources^[The three different sources need not be different websites or from different organizations. For example, three different tables from the US Census would be sufficient] for potential use in your final project. You are not bound to this decision, but choose with the rest of the semester in mind. Project 2 asks you to present one of these datasets and requires real-world data --- nothing that ships inside an R package --- with a sample size in the thousands. Project 3 sends you back to these three to develop hypotheses from one of them. The final project requires you to merge at least two datasets and fit a predictive model on the result. So: at least two of your three should share a plausible key --- the same geography, the same unit of observation, or the same time period --- and at least one should have observations numbering in the thousands.
...
For each dataset, give the name of the source, a working link to the page you would download it from, the unit of observation, and roughly how many rows it has. Then write a single short *paragraph* about what about this data interests the group and what general questions one could answer with it. Add this to the memo from Part 1, using `##` to make a new header in Rmarkdown for this section. 
...
1. Technical difficulty: Does the memo show mastery of the R and ggplot skills we've discussed thus far? Did the underlying data analysis use and expand on the tools we learned in class? Full credit requires going beyond the basic **tidyverse** filtering / grouping / summarizing. (10 points)
