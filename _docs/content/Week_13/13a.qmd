...
We can use the `str_detect` function to see that two of the four columns have commas in the entries:
...
We take this opportunity to introduce the `str_view` function, which is helpful for troubleshooting as it shows us where the matches are. Heads up: these figures were made with an older __stringr__. As of version 1.5.0, `str_view` marks _all_ the matches rather than just the first, prints in the console as `<5>'<1><0>` instead of as a highlighted box, and `str_view_all` is deprecated, so your screen will not look exactly like these pictures:
...
```{r, echo=F}
include_graphics(here('images','str_view-1.png'))
```


and `str_view_all` shows us all the matches, so `5 feet` has one match and `5'10` has three.
...
```{r, echo=F}
include_graphics(here('images','str_view-2.png'))
```
...
```{r, echo=F}
include_graphics(here('images','str_view-3.png'))
```
...
```{r, echo=F}
include_graphics(here('images','str_view-4.png'))
```
...
```{r, echo=F}
include_graphics(here('images','str_view-5.png'))
```
...
We want to extract the digits so we can then form the new version using the feet symbol `'`. These are our two groups, so we encapsulate them with parentheses:
...
```{r, echo = F}
knitr::include_graphics(here('images','str_view-6.png'))
# ![](./img/str_view-6.png)
```
...
Let's break this one down:
  ...
  - `[,\\.\\s+]` = the feet symbol: exactly one character, either a `,`, a `.`, or a space. (Watch out: inside the brackets that `+` is not a quantifier, it's a literal plus sign, which this class matches too.)
  - `\\s*` = none or more white space
...
For case 2, if we add a `'0` after the first digit, for example, convert all `6` to `6'0`, then our previously defined pattern will match. This can be done using groups:
...
We can adapt this code slightly to handle the case 1 as well, which covers the entry `5'`. Note `5'` is left untouched. This is because the extra `'` makes the pattern not match since we have to end with a digit between 4 and 7. We want to permit the 5 or 6 to be followed by 0 or 1 feet sign. So we can simply add `{0,1}` after the `'` to do this. However, we can use the none or once special character `?`. As we saw above, this is different from `*` which is none or more. We now see that the fourth entry of `s` is also converted:
...
Case 5, meters using commas, we can approach similarly to how we converted the `x.y` to `x'y`. A difference is that we require that the first digit be 1 or 2:
...
However, we can use `extract`. The regex is the same one we used above: instead of splitting on the `'`, it looks for digits on each side of it, so whatever trails the inches -- the `"` or the word `inches` -- is simply left out of the groups:
...
We see heights of 50, 51, 52, and 53. These short heights are rare and it is likely that the students actually meant `5'0`, `5'1`, `5'2`, and `5'3`. Because we are not completely sure, we will leave them as reported. The object `new_heights` contains our final solution for this case study.
...
```{r, echo=F}
knitr::include_graphics(here('images','pnas-table-s1.png'))
```
...
There are other similar functions in other R packages, such as `recode_factor` in __dplyr__ and `fct_recode` in the __forcats__ package.
...
16. Now finish it up by tidying `dat` with the gather function.
...
```{r, echo=F}
include_graphics(here('images','murders-data-wiki-page.png'))
```
...
```{r, echo=F}
knitr::include_graphics(here('images','html-code.png'))
```
...
The table we are interested in is the first one:
...
SelectorGadget^[http://selectorgadget.com/] is a piece of software that allows you to interactively determine what CSS selector you need to extract specific components from the webpage. If you plan on scraping data other than tables from html pages, we highly recommend you install it. A Chrome extension is available which permits you to turn on the gadget and then, as you click through the page, it highlights parts and shows you the selector you need to extract these parts. There are various demos of how to do this including __rvest__ author Hadley Wickham's
vignette^[https://rvest.tidyverse.org/articles/selectorgadget.html] and other tutorials based on the vignette^[https://stat4701.github.io/edav/2015/04/02/rvest_tutorial/] ^[https://www.analyticsvidhya.com/blog/2017/03/beginners-guide-on-web-scraping-in-r-using-rvest-with-hands-on-knowledge/].
...
citi_bike <- fromJSON("https://gbfs.citibikenyc.com/gbfs/en/station_information.json")
```

This downloads a list. The `last_updated` element tells you when the feed was last refreshed, as a Unix timestamp:

```{r,eval=FALSE}
citi_bike$last_updated
```

and the stations are a data table nested one level down:

```{r,eval=FALSE}
citi_bike$data$stations %>% as_tibble()
```
