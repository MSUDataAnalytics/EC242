...
We start with a single predictor example, again using `balance` as our single predictor. Note that `default_trn` has a factor variable for `default` (No/Yes). Since `R` represents factor variables as numbers (here, 1 and 2), but the order of those numbers and their mapping to the character string is arbitrary. `R` sets the first character string it sees as 1, the second as 2, etc., so if you read the data in a different order or re-make the factor variable, it can switch.

It's best to be explicit. Let's set the `default` variable to be `Yes = 1` and `No = 0`. Then it's a true binary, and `mean(default_trn)` gives us the average default rate (in the training set). We *should* do this before we split the sample, but we can do it now to both samples as long as we do the exact same thing to each.
...
We could do that, or we could use `type = "response"`
