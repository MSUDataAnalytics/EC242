...
To do this in R, we'll first use the formula from [Week 07a](/content/Week_07/07a.html#the-regression-line-1):
...
Note that the formulas from  [Principles 07](/content/Week_07/07a.html#the-regression-line-1) have a direct translation into the code. Also note that we used `summarize` on the data so that we get only **one** row of data (no more 1,000+ rows, we've summarized them using `mean` and `sd`).
