...
__Historical note__: Galton made important contributions to statistics and genetics, but he was also one of the first proponents of eugenics, a scientifically and ethically flawed philosophical movement favored by many biologists of Galton's time but with horrific historical consequences. These consequences still reverberate to this day and form the basis for much of the Western world's racist policies. You can read more about it here: [https://pged.org/history-eugenics-and-genetics/](https://pged.org/history-eugenics-and-genetics/).

## Case study: is height hereditary?

We have access to Galton's family height data through the __HistData__ package. This data contains heights on a couple hundred families: mothers, fathers, daughters, and sons. To imitate Galton's analysis, we will create a dataset with the heights of fathers and a randomly selected son of each family:
...
For other pairs, the correlation is in between -1 and 1. The correlation between father and son's heights is about 0.43:
...
## QQ plots
The above plot is called a `qq plot`. It compares two things: the quantiles of the numeric variable, and the quantiles that one would get from a normal distribution. Points on the reference line indicate ranges of the data where the empirical quantile is equal to the theoretical (normal) quantile. Points off the line indicate ranges where the numeric variable is not normal. Note that `stat_qq` puts *standard* normal quantiles on the horizontal axis, which is why the line we drew above has intercept `mean(R)` and slope equal to the theoretical standard error rather than being a 45 degree line. Small samples will be very noisy, but you should be able to see systematic deviations.
:::
...
A practical way to improve these estimates of the conditional expectations, is to define strata with similar values of $x$. In our example, we can round father heights to the nearest inch and assume that they are all 72 inches. If we do this, we end up with the following prediction for the son of a father that is 72 inches tall:
...
Note that a 72-inch father is taller than average -- specifically, (72 - `r round( mean(galton_heights$father),1)`)/`r round(sd(galton_heights$father),2)` =
`r round((72 -mean(galton_heights$father))/sd(galton_heights$father), 1)` standard deviations taller than the average father. Our prediction `r conditional_avg` is also taller than average, but only `r round((conditional_avg - mean(galton_heights$son)) /sd(galton_heights$son),2)` standard deviations larger than the average son. The sons of 72-inch fathers have _regressed_ some to the average height. We notice that the number of SDs shrinks by a factor of about 0.43, which happens to be the correlation. As we will see in the next section, this is not a coincidence.
...
The regression formula implies that if we first standardize the variables, that is subtract the average and divide by the standard deviation, then the regression line has intercept 0 and slope equal to the correlation $\rho$. You can make the same plot, but using standard units like this:
...
Correlation and the regression slope are widely used summary statistics, but they are often misused or misinterpreted. Anscombe's examples provide over-simplified cases of datasets in which summarizing with correlation would be a mistake. But there are many more real-life examples.
...
This is the regression line, with slope $m = \rho \frac{\sigma_Y}{\sigma_X}$ and intercept $\mu_Y - m\mu_X$. It is equivalent to the regression equation we showed earlier which can be written like this:
...
This implies that, if our data is approximately bivariate normal, the regression line gives the conditional expectation. Therefore, we can obtain a much more stable estimate of the conditional expectation by finding the regression line and using it to predict.

In summary, if our data is approximately bivariate normal, then the conditional expectation, the best prediction of $Y$ given we know the value of $X$, is given by the regression line.
...
Specifically, it is reduced to $\sqrt{1-\rho^2} = \sqrt{1 - 0.19}$ = 0.90 of what it was originally. We could say that father heights "explain" 10% of the variability observed in son heights.

The statement "$X$ explains such and such percent of the variability" is commonly used in academic papers. In this case, this percent actually refers to the variance (the SD squared). So if the data is bivariate normal, the variance is reduced to $1-\rho^2$ of what it was, so we say that $X$ explains $1- (1-\rho^2)=\rho^2$ (the correlation squared) of the variance.
...
$x = \{ \mbox{E}(Y\mid X=x) -$ `r round(b_1, 1)` $\} /$ `r round(m_1, 2)`.
