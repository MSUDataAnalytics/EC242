...
Next time, we'll formally cover defining our loss function (root mean squared error) and using it to tune a model. But for now, let's compare the two models above -- the "parsimonius" one where we let $Z$ be an intercept shift, and the more complex "flexible" model where we interact $X$ with $Z$ (treating $Z$ as a factor). 
...
And for root mean squared error, we finally take the square root. This looks an awful lot like the standard deviation of the residual, right? It just about it (aside from the sample $\frac{1}{N-1}$ difference).
...
Starting next time, we'll look at what we can learn when we aim only for fitting an explanatory model rather than a theoretically valid model.
