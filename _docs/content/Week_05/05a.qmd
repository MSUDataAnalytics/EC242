...
### Some new packages
We'll be using `tmaptools` geocoder today, so you'll need to install that package. And `tigris` if you didn't already. And `mapview`, which shows up later on -- I can't embed its output in the website, so those chunks don't run here, but you'll want it installed if you're playing along at home.
...
We'll go through more on finding data on Thursday. But for now, let's use some data from the HIFLD Open Database, which ~~is one of my favorite sources for spatial data put out by the DHS [HIFLD Open database](https://hifld-geoplatform.hub.arcgis.com). It has lots of government datasets that are well-organized by category. Click through, choose the "open" database, and when you find something you like, click the "Download" button.~~ 
...
GEOJSON, shapefiles, and KML files usually come with embedded projections stored as **EPSG** numbers like '4326' (incidentally, '4326' is the usual projection for GPS coordinates). Thus, the first one is usually already taken care of. If your data doesn't have a PROJ4 or EPSG number but the coordinates are all between -180 and +180, it's likely in EPSG=4326. If none of those, then the data creator should have *metadata* stating the projection. It might take some googling and some trial and error.

For mapping, you might need to *transform* your data between projections (or "reproject" it, same thing). We use `st_transform` for this. We only need to give `R` the EPSG (Geodetic Parameter Dataset) of the projection you want to end up in. As long as it's already in a known projection, `R` can re-project it. The projection is referred to by the Coordinate Reference System (CRS). `st_crs` will tell us the projection (EPSG number and a lot more) of any spatial object. If they do not match, then `R` will give an error or, worse, plot them on totally different scales - sometimes you end up with points from the US landing in the middle of the Indian Ocean! Look back at our map of gas plants and the US: that one came out fine anyway, because `ggplot` quietly re-projects every `geom_sf()` layer into the CRS of the first one. `st_join()` does no such favor -- it just errors. 
...
`gasplants` from HIFLD is in EPSG: 3857 ("WGS 84 / Pseudo-Mercator" -- note the coordinates printed above are in meters, not degrees), the map of MI is in EPSG: 4269. We can use `st_transform` on the `gasplants` data, which will reproject the points (and won't change the data at all). The data won't be any different, and the points won't look too much different
...
### Mapview
Sometimes, we want to be able to zoom in and out. `ggplot` is static, so that won't work too well. Thanks to the `leaflet` engine, the `mapview` package is stellar for exploration of spatial data. You can specify `zcol = 'NAME'` if you want to color by the `NAME` field -- `zcol` wants the column name in quotes. I can't embed this in the website, but you can run this at home. It will appear in the "Viewer" pane, not in the "Plots" pane. Unlike the static image here, you will be able to zoom and pan.
...
These watersheds clearly overlap state boundaries. So what happens if we merge them? `sf` will create a new observation (row) for every HUC-4 / State combination
...
### Summarizing 
Our `summarize` function let us collapse by groups and calculate interesting things like average (over a group or region). The neat part is that *it works on spatial data as well*. Let's look at the data again:
...
So AZ has two HUC-4's in it - Lower Colorado and Lower Colorado-Lake Mead (you can see them above). Summarize on geospatial data works just like regular data - we can `group_by(STUSPS)`, and we can `summarize()` any of the data. I threw some random data into `HUC-4` so we can summarize that. 
...
If we had wanted to just take the average (ignoring area), we'd just leave out the `w = State.HUC.area` or just use `mean`. If we had wanted to take, say, the minimum, we would use `min(randomData)` instead of `weighted.mean`. We can use whatever function we want in `summarize`, just as we did with non-spatial data.

### Choosing the right summarization

What's the "right" way to summarize data after using `st_intersection`? Well, it's going to depend on the data context. Above, we used the area-weighted mean to take the data from the smaller parts and add it up to the larger state measure.

::: {.callout-note}
# Let's discuss

What sort of data would work well being weighted by area?

What sort of data wouldn't work well if weighted by area?

What other ways could we weight data?
...
This introduces a useful concept: the *bounding box*. The bounding box is the smallest box -- given by an `xmin`, `ymin`, `xmax`, and `ymax` -- that perfectly encloses the object (even when the object is not a rectangle). The extent of the above plot is the bounding box for Nevada.
...
ourDistance
```
We get a `units` matrix, which has extra properties that allow us to convert the units. For data in a projected CRS, the units are whatever linear unit that CRS uses. Here `st_crs(ourCities.spatial)` is NAD83, whose coordinates are *degrees* -- so `sf` measures the distance on the sphere and hands it back in meters.

What if we wanted to find the closest gas plant to each city? That is akin to looking at each row, and finding the column that is the smallest, right? We will use `apply`, and we will note that the order of the columns is the same as the order in `MI.gasplants.small`, so we can use `MI.gasplants.small$name` to tell us the name of the closest gas plant. We will `apply` over each row (`MAR=1`) and use the `which.min` function, which returns the *index* number of the minimum column. (Yes, I then store it in something called `max.index`. It holds the index of the *minimum* distance. Bad name, my fault, live with it.)
...
we want to select from our distance matrix the 1st row, 3rd column; the 2nd row, 3rd column; 3rd row, 4th column; 4th row, 4th column; 5th row, 2nd column; and 6th row, 1st column. This means the row index and column index are not ranges, but are paired. Using `cbind(1:6, max.index)` makes them paired entries, and we can select specific row x column combinations that way.

### st_nearest_feature
As is common in R, there is a function that will get the closest points between two spatial objects. `st_nearest_feature` takes two geometries, and returns the *index* of the nearest feature in `y` for every point in `x`, which is what we did with the MI gas plants.
...
### Proximity-based merges
What if we wanted to identify every polygon (or point) within some distance (say, "X km") of a gasplant rather than finding the closest gasplant? Of course, we could do the above steps, and then filter the distance matrix to keep only those within X km. Or, we could look for the polygons that intersect an X km circle around a gasplant. Doing this is easy in R.

To do so, we use `st_buffer()` along with a distance in some linear measure (meters, say, rather than degrees of lat or lon). This is because a degree of longitude *changes* when we get closer to the poles. Let's find all counties in Michigan that are within 50 km of any Great Lake. To do so, we first buffer our Great Lakes by 50km. `GL` is in EPSG:4269, whose *coordinates* are degrees -- but when the data are lon/lat, `sf` does the geometry on the sphere and takes `dist` in meters anyway, so we can buffer by 50,000:

```{r}
st_crs(GL) # NAD83 / EPSG:4269 -- the axes are in degrees
GL.buffer = st_buffer(GL, dist = 50000)

ggplot(GL.buffer) + geom_sf()

```



Now, we can ask for county polygons that intersect this buffered object. We want an "inner" join (only those on the left that appear on the right, where "appear" refers to the spatial relationship):

```{r}
MI.lakeside.counties = MI.counties %>%
  st_transform(st_crs(GL.buffer)) %>%
  st_join(GL.buffer, left=F)

ggplot(MI.lakeside.counties) + geom_sf() + theme_minimal()

```

Alternatively, we could have kept all of the counties but used the joined column `FEAT_NAME`, which merges with `NA` for counties that don't touch, and *duplicates* observations for counties that touch 2 or more!

```{r}
NROW(MI.counties) #83 counties

MI.lakeside.counties = MI.counties %>%
  st_transform(st_crs(GL.buffer)) %>%
  st_join(GL.buffer, left=T)

NROW(MI.lakeside.counties)
```

What happened? For each Great Lake *polygon* within 50km, a county in the merged data got one row, so many counties got two (or more) rows. Note that the `GL` file splits some lakes into several polygons, so a county can even end up with two rows for the *same* lake:

```{r}
head(MI.lakeside.counties)
```

We need to figure out what to do with `NA` (no Great Lake) or multiple Great Lakes. Here's what we do:

```{r}
MI.lakeside.counties = MI.lakeside.counties %>%
  group_by(COUNTYFP, NAME) %>%
  summarize(NearLake = !any(is.na(FEAT_NAME)))
...
```{r}
MI.lakeside.counties %>%
  ggplot() + geom_sf(aes(fill = NearLake)) + theme_minimal() + 
  labs(fill = 'Near Great Lake')
```
