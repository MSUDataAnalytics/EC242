...
- The difference between **model parameters** and **tuning parameters**.
...
We saw previously (see the slides from the last two content days) that this risk is minimized by the **conditional mean** of $Y$ given $\boldsymbol{X}$,
...
Notice that what is returned are (maximum likelihood or least squares) estimates of the unknown $\beta$ coefficients. That is, the "learning" that takes place with a linear model is "learning" the values of the coefficients.
...
We'll start with **k-nearest neighbors** which is possibly a more intuitive procedure than linear models.^[We chose to start with linear regression because most students in the social sciences should already be familiar with the basic notion.]
...
as our estimate of the regression function at $x$. While this looks complicated, it is actually very simple. Here, we are using an average of the $y_i$ values for the $k$ nearest neighbors to $x$.
...
- In the middle plot we use $k = 5$. The blue "curve" is the estimate of the mean function for each $x$ shown in the plot.
- In the right plot we use $k = 1$. The green "curve" is the estimate of the mean function for each $x$ shown in the plot.
...
This $k$, the number of neighbors, is an example of a **tuning parameter**. Instead of being learned from the data, like model parameters such as the $\beta$ coefficients in linear regression, a tuning parameter tells us *how* to learn from data. It is user-specified. To determine the value of $k$ that should be used, many models are fit to the estimation data, then evaluated on the validation data. Using the information from the validation data, a value of $k$ is chosen. (More on this in a bit.)
...
The above "tree"^[It's really an upside down tree isn't it?] shows the splits that were made. It informs us of the variable used, the cutoff value, and some summary of the resulting neighborhood. In "tree" terminology the resulting neighborhoods are "terminal nodes" of the tree. In contrast, "internal nodes" are neighborhoods that are created, but then further split.

The "root node" is the neighborhood that contains all observations, before any splitting, and can be seen at the top of the image above. We see that this node represents 100% of the data. The other number, 0.21, is the mean of the response variable, in this case, $y_i$.
...
- `cp` or the "complexity parameter" as it is called.^[Flexibility parameter would be a better name.] This parameter determines which splits are accepted. A split must improve the performance of the tree by more than `cp` in order to be used. When we get to R, we will see that the default value is 0.01.
- `minsplit`, the minimum number of observations in a node (neighborhood) in order to consider splitting within a neighborhood. The default value is 20.

There are actually many more possible tuning parameters for trees, possibly differing depending on who wrote the code you're using. We will limit discussion to these two.^[The `rpart` function in R would allow us to use others, but we will always just leave their values as the default values.] Note that they affect each other, and they affect other parameters which we are not discussing. The main takeaway should be how they affect model flexibility.
...
Again, we are using the `Credit` data from the `ISLR` package. Note: **this is not real data.** It has been simulated.
...
After the train-test split (with 80% of the data going to train), we look at the train data.
...
crdt_knn_75 = knnreg(Rating ~ Age + Gender + Student, data = crdt_trn, k = 75)
```

Here, we fit four models to the training data. We supply the variables that will be used as features as we would with `lm()`. We also specify how many neighbors to consider via the `k` argument.
...
OK, so of these four models, which one performs best? (Where for now, "best" is obtaining the lowest validation RMSE.)
...
So, of these four values of $k$, the model with $k = 25$ achieves the lowest validation (test) RMSE.
...
Note that we re-split the data with a new seed here, so the RMSEs in this section aren't directly comparable to the KNN numbers above. We'll start by using default tuning parameters for `minsplit`, but I'm going to use a `cp` that gets us something interesting.
...
At each split, the variable used to split is listed together with a condition. If the condition is true for a data point, send it to the left neighborhood. Although `Student` is available for creating splits, we only see splits based on `Age` and `Gender`. This hints at the relative importance of these variables for prediction. More on this much later.

Categorical variables are split based on potential categories! This is *excellent*. This means that trees naturally handle categorical features without needing to convert to numeric under the hood. We see a split that puts men into one neighborhood, and women into another.
...
In other words, individuals in this terminal node are men who are between the ages of 78 and 83. (Only 3% of the data is represented here.) This is basically an interaction between `Age` and `Gender` without any need to directly specify it! What a great feature of trees.
...
Now let's fit another tree that is more flexible by relaxing some tuning parameters. Recall that by default, `cp = 0.01` and `minsplit = 20`.
...
Note that by only using these three features, we are severely limiting our model's performance. Let's quickly assess using all available predictors.
