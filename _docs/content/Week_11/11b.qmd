...
I'm going to have you form groups of 2-3 to work on live-coding. One person in the group will need to have Rstudio up, be able to share screen, and have the correct packages loaded (`wooldridge`, `rpart` and `rpart.plot`, plus `dplyr` and `skimr`). Copy the `rmse` and `get_rmse` code into a blank R script (you don't have to use RMarkdown, just use a blank R script and run from there). You'll also want to have these functions from last week loaded:
...
predict(onetree, newdata = data.frame(
  hours = 40, IQ = 100, KWW = 50, educ = 18, exper = 19, tenure = 15, age = 20, married = 1, black = 0, south = 0, urban = 1, sibs = 1))
```
**5 minutes**
