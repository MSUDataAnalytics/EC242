...
- <i class="fas fa-book"></i> Chapter 3 in [Introduction to Statistical Learning](https://hastie.su.domains/ISLR2/ISLRv2_corrected_June_2023.pdf.download.html)
...
# Previously scraped from stevetheump.com at render time. That page no longer
# serves the payroll tables, so this now comes from Lahman::Salaries, which
# ships with the package and needs no network access.
Salaries %>%
  filter(yearID == 2002) %>%
  group_by(teamID) %>%
  summarise(Payroll = sum(salary) / 10^6, .groups = "drop") %>%
  mutate(Team = reorder(teamID, Payroll)) %>%
...
```{r, echo=F, fig.cap = '(Courtesy of Cburnett^[https://en.wikipedia.org/wiki/User:Cburnett]. CC BY-SA 3.0 license^[https://creativecommons.org/licenses/by-sa/3.0/deed.en].)'}
knitr::include_graphics(here('images','Baseball_diamond1.png'))
```
...
```{r, echo=F, fig.cap = '(Picture courtesy of Keith Allison^[https://www.flickr.com/people/27003603@N00]. CC BY-SA 2.0 license^[https://creativecommons.org/licenses/by-sa/2.0].)'}
knitr::include_graphics(here('images','JumboTron.png'))
```
...
Let's start with an obvious one: HRs. Do teams that hit more home runs score more runs? We examine data from 1961 to 2001. The visualization of choice when exploring the relationship between two variables, such as HRs and runs, is a scatterplot:
...
Here again we see a clear association. But does this mean that increasing a team's BBs **causes** an increase in runs? One of the most important lessons you learn in this class is that **association is not causation.**
...
and use the formulas given above to create the regression lines, as we did in Week 7's Content, and adding the line to our plot `p` created earlier:
...
Previously, we noted a strong relationship between Runs and BB. If we find the regression line for predicting runs from bases on balls, we get a slope of:
...
We are again reminded that association is not causation. The data does provide strong evidence that a team with two more BB per game than the average team, scores `r round(bb_slope*2, 1)` more runs per game than the average team. But this does not mean that BB are the cause.
...
Also, notice that a single gets you to first base just like a BB. Those that know about baseball will tell you that with a single, runners on base have a better chance of scoring than with a BB. So how can BB be more predictive of runs? The reason this happens is because of confounding. Here we show the correlation between HR, BB, and singles:
...
Regardless, it seems that if we stratify by HR, we have approximate bivariate normal distributions for runs versus BB. Similarly, if we stratify by BB, we have approximate bivariate normal distributions for HR versus runs.
...
This model suggests that if the number of HR is fixed at $x_2$, we observe a linear relationship between runs and BB with an intercept of $\beta_0 + \beta_2 x_2$. Our exploratory data analysis suggested this. The model also suggests that as the number of HR grows, the intercept growth is linear as well and determined by $\beta_2 x_2$.
...
Trial and error is not going to work in this case. We could search for a minimum within a fine grid of $\beta_0$ and $\beta_1$ values, but this is unnecessarily time-consuming since we can use calculus: take the partial derivatives, set them to 0 and solve for $\beta_0$ and $\beta_1$. Of course, if we have many parameters, these equations can get rather complex. But there are functions in R that do these calculations for us. We will learn these next. To learn the mathematics behind this, you can consult a book on linear models.
...
The second argument is the key here. It is the `data=` argument. Here, you're telling R where to find the variables `son` and `father`. 
...
# Rebuild the HR strata here: the most recent `dat` above was the BB-strata
# subset, and this section is about the slope of runs on BBs within HR strata.
dat <- Teams %>% filter(yearID %in% 1961:2001) %>%
  mutate(HR_strata = round(HR/G, 1),
         BB_per_game = BB / G,
         R_per_game = R / G) %>%
...
We won't work through `augment` here (though you used it already to predict values in an earlier assignment); it returns the original data plus fitted values and residuals.
...
with $Y_i$ runs per game for team $i$, $x_{i,1}$ walks per game, and $x_{i,2}$ home runs per game. To use `lm` here, we need to let the function know we have two predictor variables. So we use the `+` symbol as follows:
...
We compute the per-plate-appearance rates for players available in 2002 on data from 1997-2001. To avoid small sample artifacts, we filter players with less than 1,000 plate appearances total over those five years. Here is the entire calculation in one line:
...
Here, `which.max` picks the one position the player most played, and in the case of ties it just returns the first position listed. We also remove the `OF` position which stands for outfielder, a generalization of three positions: left field (LF), center field (CF), and right field (RF). We also remove pitchers since they don't bat in the league in which the A's play.
...
So is it "jitters" or "jinx"? To answer this question, let's turn our attention to all players that played the 2013 and 2014 seasons and batted at least 130 times (the at-bat cutoff that defines rookie eligibility).
...
$$ \frac{Y - .255}{.033} = 0.46 \left( \frac{X - .261}{.032}\right) $$

Because the correlation is not perfect, regression tells us that, on average, we expect high performers from 2013 to do a bit worse in 2014. It's not a jinx; it's just due to chance. The ROY are selected from the top values of $X$ so it is expected that $Y$ will regress to the mean.
