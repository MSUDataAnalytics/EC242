...
```{r}
ggplot(mtcars, aes(x = hp, y = mpg, shape = as.logical(am), col = as.logical(am))) +
  geom_point(size = 4) + theme_minimal() + guides(shape = F) + labs(col = 'Manual\nTransmission')
```
...
First, note that `am` is already a dummy variable, since it uses the values `0` and `1` to represent automatic and manual transmissions. Often, a variable like `am` would store the character values `auto` and `man` and we would either have to convert these to `0` and `1`, or, as we will see later, `R` will take care of creating dummy variables for us. In the plot above, we use `as.logical`, which takes 0/1 and makes them `FALSE`/`TRUE`, which are treated as factor variables for plotting.


### It's just a dummy....
We've noticed that most of the blue dots (with manual transmission) have higher `mpg`. We have a dummy in our data. Taking a step back from using `hp` to explain variation in `mpg`, let's look at how much `am` does on its own:

::: {.callout-note}
## TRY IT

1. Calculate the mean mpg conditional on `am==1` and on `am==0`. Since `mpg` is a random variable -- it changes from observation to observation -- we can use our sample to learn something about this random variable. In this context, the conditional averages are our sample estimate of what *population* concept?

2. Regress `mpg` on our dummy `am` (and only our dummy, `am`. Ignore `hp` for now.) What *population* concept are we estimating with the regression?


Given your answers to (1) and (2), should we expect them to be the same?

:::



### Multivariate with a dummy

We've established what the dummy variable does on its own. Now we'll use it in a multivariate regression. To fit a more complex model using the dummy and `hp`, we do so like any other multiple regression model we have seen before.
...
```{r}
ggplot(mtcars, aes(x = hp, y = mpg, shape = as.logical(am), col = as.logical(am))) +
  geom_abline(intercept = int_auto, slope = slope_auto, col = 'red') + 
  geom_abline(intercept = int_manu, slope = slope_manu, col = 'blue') +
  geom_point(size = 4) + theme_minimal() + guides(shape = F) + labs(col = 'Manual\nTransmission')
```

We notice right away that the points are no longer systematically incorrect. The blue, manual observations vary about the blue line in no particular pattern without underestimating the observations as before. The red, automatic points vary about the red line, also without an obvious pattern.
...
Again, we see a very low p-value so we reject the null (additive model) in favor of the interaction model. The `anova()` output above tests exactly the same thing and gives the same p-value; note again that the $F$ statistic is the $t$ statistic squared.
...
Earlier when we used the `domestic` variable, it was **not** a factor variable. It was simply a numerical variable that only took two possible values, `1` for domestic, and `0` for foreign. Let's overwrite the raw `origin` column -- the numeric code we used to build `domestic` -- with a variable that stores the same information, but in a different way. First, we create an empty (all-`NA`) variable of type `character`. Then, we update it. Yes, we could also do this with `ifelse` or `case_when`:
...
If you want to see the dummy variables `R` builds, run `model.matrix(mpg ~ disp * origin, data = autompg)` and look at the column names.
...
- $\beta_0 + \beta_3$ is the average `mpg` for an 8 cylinder car with 0 `disp`.
...
1. Be prepared to interpret the "baseline" coefficient on `GrLivArea`, as well as one of the coefficients on the interaction.
...
A quick note here: the full model does still "fit better." Notice that it has a smaller mean squared error than the null model, which means the full model makes smaller (squared) errors on average.
