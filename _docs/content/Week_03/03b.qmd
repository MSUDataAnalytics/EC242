...
```{r puzzle-data, echo=FALSE, message=FALSE, warning=FALSE}
library(tidyquant)
AAPLNFLX = tq_get(c("AAPL","NFLX"), from = '2021-01-01', to = '2021-08-01', get = 'stock.prices')
```

`AAPLNFLX` holds the daily closing price of two stocks, Apple and Netflix, from January through July 2021. One row per stock per day.
...
What's wrong with this plot? We'll come back to it.
...
That's the puzzle from the top of the page. For every date, we have two values - the NFLX and the AAPL value, so each day has a vertical line drawn between the two prices. This is nonsense, especially since what we want to see is the history of NFLX and AAPL over time.

**Try it:** fix it with one change inside `aes()`.
