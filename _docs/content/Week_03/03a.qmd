...
<!-- - Reveal the code on the re-order example towards the end. (DONE) -->
<!-- Removed a dead link to bconnelly.net blog on colorblind friendly colors. (DONE) -->

<!-- - The "align comparable things" notion is spread out all over the end. Focus the purpose of faceting, THEN introduce the alignment concept. -->
...
- Nominate **one** person in your group to e-mail me at {{< var email.instructor >}} with the subject **[EC242] Groups**.
- On the e-mail, cc- the other two members of the group
- In the e-mail, list the names and emails of all three members.
- Give me the team name you'd like to have as well; be creative, or I will make one up for you.

Remember, only one person per group should send this email. Groups must be 3 people exactly. Groups will be the same for the semester (all 4 projects).

On Thursday, I will assign all remaining students to a group. *No changes will be made and there is no room for late group formation.* You will receive an email letting you know your group assignment on or before Saturday.
...
### Load some packages
We will be using these libraries---note the addition of __gridExtra__. I trust you to notice and install new packages from here on out:
...
2. The number of information carrying (variable) dimensions depicted should not exceed the number of dimensions in the data. Graphics must not quote data out of context.

3. Clear, detailed and thorough labeling should be used to defeat graphical distortion and ambiguity. Write out explanations of the data on the graph itself.  Label important events in the data.

4. Viewers eyes will follow a predictable path. Visual cues serve to light up that path. 

5. The representation of numbers, as physically measured on the surface of the graph itself, should be directly proportional to the numerical quantities represented. 

6. Empty space is informative. Do not fill every inch, but let the absence of information guide the viewer.

7. When possible (and in a visually suitable way), show the data.
...
```{r}
knitr::include_graphics(here('images','pushpull.jpg'))
```
...
browsers1 = copy(browsers) # for later callout
```
...
Notice how much easier it is to see the differences in the barplot. In fact, we can now determine the actual percentages by following a horizontal line from the top of each bar over to the y-axis.
...
```{r, fig.cap = 'Image courtesy of Karl Broman'}
knitr::include_graphics(here('images','fig8b.png'))
```
...
```{r, fig.cap = 'Images courtesy of Karl Broman'}
knitr::include_graphics(here('images','fig1e.png'))
knitr::include_graphics(here('images','fig2d.png'))
```
...
When using barplots, it is misinformative not to start the bars at 0. This is because, by using a barplot, we are implying the length is proportional to the quantities being displayed -- a natural **visual cue**. By avoiding 0, relatively small differences can be made to look much bigger than they actually are. This approach is often used by politicians or media organizations trying to exaggerate a difference. Below is an illustrative example used by Peter Aldhous in this lecture: [https://web.archive.org/web/20210101194828/https://paldhous.github.io/ucb/2016/dataviz/week2.html](https://web.archive.org/web/20210101194828/https://paldhous.github.io/ucb/2016/dataviz/week2.html). 

```{r, fig.cap ='(Source: Fox News, via Media Matters^[http://mediamatters.org/blog/2013/04/05/fox-news-newest-dishonest-chart-immigration-enf/193507]'}
knitr::include_graphics(here('images','class2_8.jpg'))
```
...
```{r, fig.cap = '(Source: Fox News, via Flowing Data^[http://flowingdata.com/2012/08/06/fox-news-continues-charting-excellence/].)'}
knitr::include_graphics(here('images','Bush-cuts.png'))
```
...
```{r, fig.cap='(Source: Venezolana de Televisión via Pakistan Today^[https://www.pakistantoday.com.pk/2018/05/18/whats-at-stake-in-venezuelan-presidential-vote] and Diego Mariano.)'}
knitr::include_graphics(here('images','venezuela-election.png'))
```
...
Note that in the plot on the left, which includes 0, the space between 0 and 46 adds no information and makes it harder to compare the between and within group variability.
...
```{r, fig.cap = '(Source: The 2011 State of the Union Address^[https://www.youtube.com/watch?v=kl2g40GoRxg])'}
knitr::include_graphics(here('images','state-of-the-union.png'))
```
...
In our murders data (below), what sort of information might we want to communicate? There are only a few variables here, so we're pretty limited on what we can show. 
```{r, echo=T}
library(dslabs)
data(murders)
head(murders)
...
<summary><strong>Don't open this till we've discussed it in class.</strong></summary>
...
bdiff = browsers1 %>%
  group_by(Browser) %>%
  dplyr::summarize(YOY = Percentage[Year==2015] - Percentage[Year==2000])

ggplot(bdiff, aes(x = Browser, y = YOY)) + geom_bar(stat = 'identity') +
  labs(x = 'Browser', y = 'Change in Share 2000-2015')
```
...
```{r reorder-boxplot-example, out.width="100%", echo=TRUE}
past_year <- 1970
p1 <- gapminder %>%
...
The average of each group is represented by the top of each bar and the antennae extend out from the average to the average plus two standard errors.  If all the viewer receives is this plot and has no other knowledge of human heights, they will have little information on what to expect if they meet a group of human males and females. The bars go to 0: does this mean there are tiny humans measuring less than one foot? Are all males taller than the tallest females? Is there a range of heights? Our viewer can't answer these questions since we have provided almost no information on the height distribution.
...
In these histograms, the **visual cue** related to decreases or increases in height are shifts to the left or right, respectively: horizontal changes. Aligning the plots vertically helps us see this change when the axes are fixed. First, here they are side by side again with `facet_grid(. ~ sex)`, and then stacked with `facet_grid(sex ~ .)`:
...
If we want the more compact summary provided by boxplots, we then align them horizontally since, by default, boxplots move up and down with changes in reported height. Following our _show the data_ principle, we then overlay all the data points:
...
### Facet grids
As the name implies, `facet_grid` can make more than just side-by-side plots. If we specify variables on both sides of the "~", we get a grid of plots.
...
The design maven Edward Tufte emphasizes the need for clarifying and empty space. Resist the urge to pack everything into a small layout. Especially in digital formats, space can be inexpensive, and can help attract the eye to your work.
...
```{r boxplot-adjacent-comps-with-color, echo=TRUE}
 dslabs::gapminder %>%
  filter(year %in% c(1970, 2010) & !is.na(gdp)) %>%
...
```{r}
knitr::include_graphics(here('images','Colorblind_example.png'))
```
...
The easiest way of getting good-looking (e.g. non-default) colors is the `scale_fill_viridis_d` function, which "inherits" (takes the place of and has the properties of) `scale_fill_discrete`. Viridis has eight color palettes -- options A through H -- and each is designed to be used to maximize the differentiation between colors. We'll look at the first four.
...
We can also use a custom palette, like the colorblind palette from before (see above where we defined `color_blind_friendly_cols`). We just need to use the `type=` argument and give ggplot our color-blind friendly palette of colors. If the palette has fewer entries than we have (N) distinct categories, R reverts to the default.
...
Let's plot one country's GDP by year, but have the color indicate the life expectancy. Whenever you map a continuous variable to a color or fill aesthetic, you get the default color ramp -- dark-to-light blue:
...
We can see that GDP per capita went up, then down in 1989 (the Romanian Revolution and the collapse of communism there), then up after that. The color ramp tells us that life expectancy reached 75 years near the end, and it certainly improved in the post-2000 era.

We can set some of the points on the ramp manually - here, the ramp starts at dark blue and ends at light blue, but what if we wanted to start at red, end at blue, and cross white in the middle? Easy! We use `scale_color_gradient2` and specify the colors for low, mid, and high, and specify the midpoint at 72.5 years.
