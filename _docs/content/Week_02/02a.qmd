...
1. The pipe `%>%` can be used to perform operations sequentially without having to define intermediate objects. Start by redefining `murders` to include rate and rank.
...
we will receive an error telling us that `range` must be size 1, not 3 (dplyr also suggests `reframe()` as the fix). With the function `summarize`, we can only call functions that return a single value. In later sections, we will learn how to deal with functions that return more than one value.
...
Note that base R (without packages loaded) has a function with a very similar name, `data.frame`, that can be used to create a regular data frame rather than a tibble.  One other important difference is historical, but you will meet it in old code: in R before version 4.0.0, `data.frame` silently coerced characters into factors. That default is gone, so today you get a character column:
...
You will still see the old fix scattered around, the rather cumbersome argument `stringsAsFactors`, which these days is redundant:
```{r}
grades <- data.frame(names = c("John", "Juan", "Jean", "Yao"),
                     exam_1 = c(95, 80, 90, 85),
                     exam_2 = c(90, 85, 85, 90),
...
One of the advantages of using the pipe `%>%` is that we do not have to keep naming new objects as we manipulate the data frame. As a quick reminder, if we want to compute the median murder rate for states in the South, instead of typing:
...
A particularly useful __purrr__ function for interacting with the rest of the tidyverse is `map_df`, which always returns a tibble data frame. However, the function being called needs to return a tibble, a named list, or a *named* vector. Since `compute_s_n` returns a bare unnamed number, the following code errors with `Argument 1 must be a data frame or a named atomic vector`:
...
The `case_when` function is useful for vectorizing conditional statements. It is similar to `ifelse` but can output any number of values, as opposed to just the two branches `ifelse` gives you. This means we can avoid long nested `ifelse` functions. Here is an example splitting numbers into negative, positive, and 0:
...
Your lab will require some reshaping of data, so make sure you read this section closely.
...
#| fig-alt: "Plot showing South Korean fertility rate (green points) and German fertility rates (red points) from 1960 to 2015. South Korean rates are much higher in 1960, while German rates are lower at around 2.5. Over time, German fertility rates decline to around 1.5, while South Korean drop from 6 to under 1.5 around 2000, remaining low through 2015."
...
We can start the data wrangling with the `pivot_longer` function, but we should no longer use the column name `year` for the new column since it also contains the variable type. We will call it `key` for now -- that was the default back when this function was called `gather()`; `pivot_longer` itself defaults to `name`:
...
The data is now in tidy format with one row for each observation and four variables: country, year, fertility, and life expectancy.
