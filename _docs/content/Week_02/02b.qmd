...
The same paper pulled the same trick on a completely different subject. In 2022
it worked out [when Elon Musk
...
We can't redo their exact analysis --- X now charges for the data. But the same
trick works anywhere people post in public, so here is 2026, on Bluesky: two
...
Three accounts, three completely different animals. The cleanest way to see it
is to compare each account's quietest hour against its busiest one.

The **Associated Press** barely has a night. Its slowest hour still carries
just under a quarter of the traffic of its busiest, nineteen percent of
everything it posts goes out between midnight and 6am, and the weekend only
dents it --- 38 posts on a weekday, 29 on a Saturday. That is what a wire
service with staff in every timezone looks like from the outside.

The **New York Times** posts *more* than the AP does in this window, and yet the
shaded band is nearly empty: its 4am hour runs at about an eighth of its noon
hour. The weekend is the sharper tell --- 63 posts on a weekday, 37 on a
Saturday. The vertical streaks are days when something happened.

**George Takei** --- one man, 1.3 million followers --- has the shape of a
person with a schedule. His quietest hour is six percent of his busiest,
essentially nothing runs before 10am or after 9pm Pacific, and Saturdays are
half-off (15 posts a weekday, 7 on a weekend day). That is not a man firing off
thoughts as they occur to him. That is a job, and quite possibly somebody
else's.

None of that came from asking anyone anything. It came from seventeen thousand
timestamps and about a hundred lines of code, which you can
[read here](https://github.com/MSUDataAnalytics/EC242/blob/FS26/code/posting-clock.R).
You are not expected to be able to write that yet. Strip out the hour I spent
fussing over the color of the paper and what remains is `read_csv`, a timezone
conversion, `count`, and two geoms --- and the geoms are what the rest of today
is about.
...
Three states, three completely different shapes: Ohio has a cluster in 2023 and
then nothing, Texas rumbles along through 2025, and South Carolina sits flat for
four years and then goes nearly vertical in January 2026.
