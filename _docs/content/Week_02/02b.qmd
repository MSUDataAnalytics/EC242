...
Each set of tasks asks you to learn about an aesthetic and put it into action with the `murders` data. We'll leave about 5 minutes to do the task, then have you come back and share your results with the class.
...
5. `geom_label` and the `fill` aesthetic
    a. Looking at `?geom_label` (which is the same help as `geom_text`), we note that "The `fill` aesthetic controls the background colour of the label".
