...
<!-- Note for Ben: I bumped up participation so it's now 6% of the grade (30 points, 5 points each for the first 6, then 1 point each (1/5th of a percent each) for additional up to 5 more points). Labs are worth a little less (12 points each, 13x12=156) and weekly writings are worth less (6 each x 12 = 72). I added a Group Project 2 (in-class presentation) during Fall Break. I've changed the check-scorint figures to reflect 12 points, and removed the "check-plus" value > 100% so that we get a better grade distribution. -->

<!-- Another note for Ben: PLS202 actually does almost everything viz we do. Students found it duplicative. They also do geospatial stuff. I think this is relatively new. At the same time, students have expressed more need for in-class coding (even though they seem to not participate when we do it -- likely they need more bite-sized coding tasks). Therefore, I'm cutting 1 week of viz and will use the time to add more in-class coding. -->

<!-- On AI: I asked students to do week 1's writing on whether or not they want to learn to code using AI, or want to learn to code and use AI to help. Largely the latter. They almost 100% recognized the importance of understanding the code first (and many recognized that there would be little innovation if nobody understood the code). So...I'm sticking with the same AI policy, but getting more time from cutting down Viz. -->
...
This course meets {{< var meeting.time >}} in {{< var meeting.location >}}. This class is in-person and will not have an online component.


### Course Structure
Here's how each week will work: before class on Tuesday, you'll read the first entry under [content](/content/) for the week according to our [course schedule](/schedule.html). On Tuesday, you'll come to lecture and participate in the discussion. Before Thursday, you'll read the second [content](/content/) entry for the week, and on Thursday you'll come to lecture ready to participate. On Saturday night by 11:59pm, you'll turn in your [Weekly Writing assignment](/assignment/#weekly-writings) responding to the weekly writing prompt **given during class on Tuesday or Thursday**, rendered to PDF by RMarkdown and using the proper template (see [assignments](/assignment)). By Monday at 11:59pm, you'll turn in your [Lab assignment](/assignment/#lab-assignments), also using the RMarkdown template. 

You'll repeat this each week until we're out of labs. If a holiday occurs on a due date, then the assignment or lab is due the next non-holiday day at 11:59pm or as otherwise noted. You'll also work with your assigned group on the group projects. We'll assign groups after the drop deadline has passed.
...
### Office hours - {{< var names.instructor >}}

The Great State of Michigan subsidizes every MSU student by approximately $6,354. Even if you are paying full tuition, Michiganders are contributing. Thus, you have resources available to you that are paid for by the taxpayers, including my 96-year-old Grandpa. **It is reasonable to expect you to use these resources. Office hours are one of those resources. Use them.**

- {{< var names.instructor >}} office hours: **{{< var officehours.instructor >}}** 


Office hours are held in my office at {{< var officehours.instructoroffice >}}. If you have a quick question right after class, grab me on the way out and we can walk-and-talk; otherwise come by during the scheduled hours.

I will check Slack throughout the week and use it as an always-on avenue of communication and help. 
...
### Slack
We will use Slack as a forum for asking questions about the course, including course policies and, primarily, help questions with `R`. Students are encouraged to help answer each others' questions, and to use the forum as a first-step for seeking help. I will monitor Slack and answer questions regularly. You must join our Slack channel with this link: {{< var slack.invite >}} by the end of Week 2 -- it counts for 1 point of participation and is mandatory. Once you have joined, bookmark our {{< var slack.link >}}.
...
::: {.callout-tip title="My DM's are closed"}
Please ask your question in one of the coursewide channels so that your classmates can also see the answer and chime in. You will receive a faster reply that way.
:::
...
**Me:** My primary area of expertise is economics. {{< var aboutme.instructor >}}
...
The course website can be found at [https://ec242.netlify.app](https://ec242.netlify.app) (but you know that. You're on it right now.)

The second required reading is the  [**{{< fa book >}} Introduction to Statistical Learning (2nd Ed)**](https://www.statlearning.com/), which is available free online (you can buy a paper copy if you want, and you hate trees).

All of the readings and software in this class are **free**. There are free online version of all the texts including [**{{< fa book >}} Introduction to Statistical Learning (2nd Ed)**](https://www.statlearning.com/) and `R` / RStudio are free (don't pay for RStudio). We will reference outside readings and there exist paper versions of some "books" but you won't need to buy anything^[If you've got money to burn, you can buy me a burrito.]
...
You will do all of your analysis with the open source (and free!) programming language [`R`](https://cran.r-project.org/). You will use [RStudio](https://www.rstudio.com/) (which is undergoing a slow-mo rebrand to "Posit" while the functionality remains the same) as the main program to access R. **Think of `R` as an engine and RStudio as a car**---`R` handles all the calculations produces the actual statistics and graphical output, while RStudio provides a nice interface for running `R` code.
...
You can [find instructions for installing `R`, RStudio/Posit, and all the tidyverse packages here.](/resource/install.html) And you may find some other goodies.
...
Fortunately there are tons of online resources to help you with this beyond our course Slack. Two of the most important are [StackOverflow](https://stackoverflow.com/) (a Q&A site with hundreds of thousands of answers to all sorts of programming questions) and [RStudio Community](https://community.rstudio.com/) (a forum specifically designed for people using RStudio and the tidyverse (i.e. you)).
...
assignments <- tribble(
  ...
  12 * 6 , "Weekly Writings (14-2 x 6 ea), drop two lowest",
  13 * 12 , "Labs (14-1 x 12 ea), drop one lowest",
  50      , "Project 1",
  40      , "Project 2",
  50      , "Project 3",
  135     , "Final project"
...
grading <- tribble(
  ...
  "4.0",  "95-100%", "2.0",  "72-76%",
  "3.5", "88-94%",  "1.5", "66-71%",
  "3.0", "83-87%",  "1.0", "58-65%",
  "2.5",  "77-82%", "0.0",  "<58%",
)

labsParticipationWriting = sum(assignments %>% dplyr::filter(grepl(Assignment, pattern = 'Participation|Writings|Labs')) %>% pull(Points))/sum(assignments$Points)
everythingElse = 1-labsParticipationWriting
```

Your grade in this course will be based on attendance/participation, labs, weekly writings, and multiple group projects.

The general breakdown will be approximately `r scales::percent(labsParticipationWriting)` for labs, participation, and weekly writings, and `r scales::percent(everythingElse)` for projects (see below for specific details). The primary focus of the course is a final project; this requires "mini-projects" to ensure you're making satisfactory progress. 

Weekly writings are intended to be an easy way to get some points. Labs will be short homework assignments that require you to do something practical using `R`. You must have access to computing resources and the ability to program basic statistical analyses and use `ggplot`, as is required in PLS202. If you are unprepared to implement basic statistical coding, please take (or retake) PLS202. I highly encourage seeking coding advice from those who instruct computer science courses – it’s their job and they are better at it than I am. I’ll try to provide a good service, but I'm really not an expert in computer science.
...
### Dropping your lowest scores {#sec-dropone}
...
`4.0` Turned in all assignments with superb effort, showed exceptional mastery of the tools on all projects, produced memorable projects, and participated extensively. Note that the bar is very high for a 4.0.

`3.5` Turned in all assignments with excellent effort, worked hard on the projects, was proud of final product, and participated regularly.

`3.0` Turned in all assignments with good effort, worked hard on the projects, and participated regularly.

`2.5` Turned in nearly all assignemnts with reasonable effort, worked hard on the projects, and participated occasionally.

`2.0` Turned in most assignments, participated in producing the projects, and had some class participation.

`< 2.0` Very little effort, missed multiple assignments, worked very little on the projects and rarely participated.

...of course, failing to turn in assignments or skirting effort on the projects can lead to a grade dramatically **lower** than just a 2.0.

### Rounding

All grade ranges are applied after rounding to the nearest integer. Please do not ask for special rounding favors. There are many ways to gain extra points in this class via participation. Place your efforts there.
...
Participation can take many forms. Most preferred is active participation during class. I will consistently ~~bribe~~ give participation credit points for answering specific questions in class, which I will clearly state as being for-credit. Wrong answers get the same credit as right answers. We are here to learn. If you knew everything already, you wouldn't be in the class. In the latter half of the semester, we will work in groups in class on small coding exercises and will share results at the end. Sharing your group's results and code will also count towards participation.

We are a sizable class, so participation points will be awarded using EC slips which you will be able to turn into me (with your name on it) at the end of class. That way, I don't slow down the class trying to write down everyone's names. Each of the slips is worth one percentage point of your grade in the participation category on D2L. If you go beyond that (and I hope you do), each slip is worth 1/5th of the credit, with a cap of 5 percentage points of extra credit.
...
For the purposes of learning `R` coding, data cleaning and processing, visualization, and other coding applications of R, you are fully permitted to use ChatGPT or other generative AI models **provided you do the following**:

1. Indicate such a use at the top of your problem set, 

2. Include a comment `#Used Generative AI` in each code chunk. When “stuck” on coding.

3. Add a comment after **each line** describing briefly the purpose of that line in a manner that clearly illustrates your understanding of that line of code. For every line of code. 

Use of generative AI without all three of the preceding requirements constitutes a serious violation of the MSU Spartan Code of Honor and an Academic Dishonesty Report *will* be filed.
...
Generative AI should be used in individual settings -- a line or two of code geared towards a specific task that is part of a larger work. Under no circumstances are you to submit the entire text (or any major portion) of an assignment to any Generative AI.
...
Michigan State University is committed to providing equal opportunity for participation in all programs, services and activities. Requests for accommodations by persons with disabilities may be made by contacting the Resource Center for Persons with Disabilities at 517-884-RCPD or on the web at rcpd.msu.edu. Once your eligibility for an accommodation has been determined, you will be issued a verified individual “Letter of Accommodation”. Please present this letter to me at the start of the term and/or two weeks prior to the accommodation date (lab, project, etc). Requests received after this date will be honored whenever possible.
...
This course structure and content has been improved greatly by {{< var names.instructorother >}}. All remaining errors are my own.
